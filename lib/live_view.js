...
LiveView.prototype.set = function(name, value) {
  ...
  if(arguments.length !== 2) {
    obj = name
    if(typeof obj.get === "function" && typeof obj.keys === "function") {
      obj.load(function() {
        _(obj.keys()).each(function(key) {
          that.set(key, obj.get(key))
        })
        _(obj.oneAssocs()).each(function(assoc) {
          var assocContainer = that.getElementFromName(assoc, that.context)
          if(assocContainer.length) {
            obj.assoc(assoc, function(associated) {
              that.subViews[assoc] = new LiveView(assocContainer, associated, function() {}, that.scope.concat(assoc))
            })
          }
        })
        _(obj.manyAssocs()).each(function(assoc) {
          var assocContainer = that.getElementFromName(assoc, that.context)
          obj.assoc(assoc, function(associated) {
            that.set(assoc + "Count", associated.length)
            if(assocContainer.length) {
              that.subViews[assoc] = new LiveViewCollection(assocContainer, associated, assoc, that.scope)
            }
          })
        })
      })
