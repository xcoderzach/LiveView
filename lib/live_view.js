...
LiveView.prototype.initialize = function(template, data, callback) {
  ...
  if(_.isArray(template)) {
    ...
    this.context = $(template).clone(true)
...
LiveView.prototype.changeTemplate = function(template) {
  ...
  _(this.data).each(function(value, key) {
    that.set(key, value)
  })
  window.derpyHerp = this
...
LiveView.prototype.getElementFromName = function(name) {
  ...
  _(this.allTemplates).each(function(context) {
    ...
    _(names).each(function(name) {
      elements = elements.add(context.find("." + name))
    })
  })
...
LiveView.prototype.set = function(name, value) {
  ...
  if(arguments.length !== 2) {
    ...
    this.data[name] = _.extend(this.data[name], value)
    if(value.content) {
      this.changeTemplateFromPolymorphicKey(name, value.content)
    }
  }
}
...
LiveViewCollection.prototype.insert = function(document, index) {
  ...
  if(!_.isArray(document)) {
    ...
    id = document.id
    view = new LiveView($.makeArray(this.templates), document, function() {}, this.scope)  
