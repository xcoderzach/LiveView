      ...
      _(obj.attrs).each(function(memo) {
        _(names).each(function(name) {
          ...
          if(memo.value.match(regex)) {
            ...
            //if the value is a boolean false, we should set the attr to true
            //or false useful for check/radio buttons
            if(typeof value.visible !== "undefined") {
              element.attr(memo.name, value.visible)
            } else {
...
LiveViewCollection.prototype.insert = function(document, index) {
  ...
  if(!_.isArray(document)) {
    ...
    id = document.id
    view = new LiveView($.makeArray(this.templates.clone()), document, function() {}, this.scope)  
